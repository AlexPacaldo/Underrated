 ...
 */
import { productPreviewLabels, type ProductPreviewField, type ProductPreviewMode } from "@/lib/productPreview";
...
export default function PreviewField({ field, label, preview, className = "", children }: { field: ProductPreviewField; label?: string; preview?: ProductPreviewMode | null; className?: string; children: ReactNode }) {
  ...
  return <div data-product-field={field} data-product-anchor={field} onClickCapture={(event) => { event.preventDefault(); preview.onSelect(field); }} className={`group/preview relative ${className} ${active ? "z-10 cursor-pointer outline-2 -outline-offset-2 outline-[#ff5a36]" : "cursor-pointer outline-2 -outline-offset-2 outline-dashed outline-white/25 hover:outline-[#ff5a36]"}`}>
    {children}
    <span className={`pointer-events-none absolute left-2 top-2 z-30 border px-1.5 py-0.5 text-[9px] font-black uppercase tracking-[.16em] transition ${active ? "border-[#ff5a36] bg-[#ff5a36] text-black" : "border-[#ff5a36] bg-black/80 text-[#ff5a36] opacity-0 group-hover/preview:opacity-100"}`}>{label ?? productPreviewLabels[field]} / click to edit</span>
  </div>;
}
