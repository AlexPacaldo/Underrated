...
import { productPreviewFields, productPreviewLabels, productPreviewPath, productPreviewProductMessage, productPreviewReadyMessage, readPreviewField, readPreviewMetrics as readProductPreviewMetrics, type ProductPreviewField, type ProductPreviewMetrics } from "@/lib/productPreview";
...
function ProductEditor({ product, onSaved, onCancel }: { product: Product; onSaved: () => Promise<void>; onCancel: () => void }) {
  ...
  const [metrics, setMetrics] = useState<ProductPreviewMetrics>({ height: 1500, offsets: {} });
  ...
  const handlePreviewMessage = useCallback((data: unknown) => {
    ...
    const next = readProductPreviewMetrics(data);
    if (next) setMetrics(next);
  }, []);

  // The stage is the scroll container for the scaled frame, so panning the frame means moving its scrollTop.
  ...
  const panTo = useCallback((top: number) => {
    const node = stageRef.current;
    if (!node) return;
    node.scrollTo({ top: Math.max(0, top), behavior: "smooth" });
  }, [stageRef]);

  const panToField = useCallback((field: ProductPreviewField | null) => {
    if (!field) { panTo(0); return; }
    const offset = metrics.offsets[field];
    if (offset === undefined) return;
    panTo(offset * scale - 12);
  }, [metrics.offsets, panTo, scale]);

  // Scrubbing moves the frame directly, and the slider follows whenever the stage is scrolled by hand.
  const panToRatio = useCallback((ratio: number) => {
    panTo(scrollRange() * Math.min(1, Math.max(0, ratio)));
  }, [panTo, scrollRange]);
  ...
  useEffect(() => {
    ...
    const track = () => {
      ...
      if (range <= 0) return;
      const next = Math.round((node.scrollTop / range) * 200) / 200;
      setPosition((current) => (Math.abs(current - next) < 0.005 ? current : next));
    };
    node.addEventListener("scroll", track, { passive: true });
    // The stage box alone does not resize when the frame gets taller, so the frame wrapper is watched too.
    const observer = new ResizeObserver(track);
    observer.observe(node);
    if (node.firstElementChild) observer.observe(node.firstElementChild);
    return () => { node.removeEventListener("scroll", track); observer.disconnect(); };
  }, [scrollRange, stageRef]);
