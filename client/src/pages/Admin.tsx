...
function ProductEditor({ product, onSaved, onCancel }: { product: Product; onSaved: () => Promise<void>; onCancel: () => void }) {
  ...
  // Framing: pick which photo to adjust, then set its object-position in the 3x3 grid.
  const [framing, setFraming] = useState<string | null>(null);
  const framingPhoto = framing && gallery.includes(framing) ? framing : gallery[0];
  const setPositionFor = (pos: ProductImagePosition) => {
    if (!framingPhoto) return;
    setDraft((current) => ({ ...current, imagePositions: { ...current.imagePositions, [framingPhoto]: pos } }));
  };
