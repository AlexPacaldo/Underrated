...
import { productPreviewPath, productPreviewProductMessage, productPreviewReadyMessage, readPreviewField, readPreviewProductHeight, type ProductPreviewField } from "@/lib/productPreview";
...
function ProductEditor({ product, onSaved, onCancel }: { product: Product; onSaved: () => Promise<void>; onCancel: () => void }) {
  ...
  const addImage = (value: string) => setDraft((current) => ({ ...current, ...normalizeImages(current.image, [...(current.images ?? []), value]) }));
  const removeImage = (value: string) => setDraft((current) => ({ ...current, ...normalizeImages(current.image, (current.images ?? []).filter((item) => item !== value)) }));
  const promoteImage = (value: string) => setDraft((current) => ({ ...current, ...normalizeImages(value, [current.image ?? "", ...(current.images ?? [])]) }));
  ...
  const upload = async (files: FileList | null) => {
    ...
    try {
      ...
      setDraft((current) => ({ ...current, ...normalizeImages(current.image, [...(current.images ?? []), ...paths]) }));
