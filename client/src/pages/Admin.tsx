...
function ProductEditor({ product, onSaved, onCancel }: { product: Product; onSaved: () => Promise<void>; onCancel: () => void }) {
  ...
  const handleDragStart = (event: React.PointerEvent, currentX: number, currentY: number) => {
    event.preventDefault();
    setDragStart({ x: event.clientX, y: event.clientY, posX: currentX, posY: currentY });
  ...
  const handleDragMove = (event: React.PointerEvent) => {
    ...
    setDraft((cur) => ({ ...cur, imagePositions: { ...cur.imagePositions, [framingPhoto]: `${Math.round(nextX)}% ${Math.round(nextY)}%` } }));
  };
