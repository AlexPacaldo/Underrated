...
// Framing: pick a photo, then drag it inside the frame to set the crop.
  ...
  const handleDragMove = (event: React.PointerEvent) => {
    ...
    const nextX = Math.max(0, Math.min(100, dragStart.posX + dx * 0.5 / zoom));
    const nextY = Math.max(0, Math.min(100, dragStart.posY + dy * 0.5 / zoom));
