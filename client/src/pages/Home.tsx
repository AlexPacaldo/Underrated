...
export default function Home() {
  ...
  return (
    <>
      <section className="relative min-h-[790px] overflow-hidden bg-[#090a0b] pt-[68px] sm:min-h-[850px]">
        ...
        <div className="relative mx-auto flex min-h-[722px] max-w-[1440px] flex-col justify-end px-4 pb-8 pt-20 sm:px-6 sm:pb-10 lg:min-h-[782px] lg:px-9">
          <div className="max-w-[680px]">
            <p className="reveal-up text-[10px] font-bold uppercase tracking-[.2em] text-[#ff5a36]">Drop 03 / Contact points</p>
            <h1 className="reveal-up mt-4 font-display text-[clamp(5rem,11vw,10.6rem)] uppercase leading-[.68] tracking-[-.065em] text-white">Make the<br /><em className="text-[#ff5a36]">cockpit</em><br />yours.</h1>
            <p className="reveal-up mt-7 max-w-sm text-sm leading-6 text-white/64 sm:text-base">Small parts change the whole build. Find the grip, color, and hardware that reads like you.</p>
      ...
      <section id="featured" className="border-t border-white/15 bg-[#111214] px-4 py-16 sm:px-6 lg:px-9 lg:py-24"><div className="mx-auto max-w-[1440px]"><div className="mb-9 flex items-end justify-between border-b border-white/15 pb-5"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#ff5a36]">Current hardware</p><h2 className="mt-3 font-display text-6xl uppercase leading-[.75] tracking-[-.05em] text-white sm:text-7xl">The <em>drop.</em></h2></div><span className="hidden text-[10px] font-bold uppercase tracking-[.16em] text-white/35 sm:block">04 highlighted parts</span></div><div className="grid gap-x-4 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 lg:gap-x-5">{products.filter((product) => product.featured).map((product) => <ProductCard key={product.id} product={product} />)}</div></div></section>

      <section className="relative overflow-hidden bg-[#eae6df] text-[#0c0d0e]"><div className="mx-auto grid max-w-[1440px] lg:grid-cols-[.92fr_1.08fr]"><div className="flex min-h-[510px] flex-col justify-between px-4 py-12 sm:px-6 lg:min-h-[650px] lg:px-9 lg:py-16"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#ff5a36]">Why it matters</p><h2 className="mt-5 max-w-md font-display text-[clamp(4.6rem,8vw,8.2rem)] uppercase leading-[.68] tracking-[-.06em]">Ride the<br /><em>details.</em></h2></div><div className="max-w-sm border-t border-black/25 pt-5"><p className="text-sm leading-6 text-black/65">The parts you reach for should earn their space. We make visual decisions feel physical: a sharper profile, a better grip, a color that finds the light.</p><Link href="/about" className="mt-6 inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.16em] text-black hover:text-[#ff5a36]">Our design attitude <ArrowRight size={14} /></Link></div></div><div className="relative min-h-[460px] overflow-hidden bg-[#0c0d0e]"><img src="/manus-storage/underrated-cockpit_6568441b.jpg" alt="Graphite road bike cockpit with wrapped bars and stem" className="absolute inset-0 h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-black/45 via-transparent to-transparent" /><p className="absolute bottom-7 left-6 max-w-[320px] font-display text-5xl uppercase leading-[.75] text-white sm:left-9 sm:text-6xl">Made to<br /><em className="text-[#ff5a36]">be noticed.</em></p><span className="absolute right-6 top-6 text-[10px] font-bold uppercase tracking-[.18em] text-white/65">The new cockpit / 01</span></div></div></section>
